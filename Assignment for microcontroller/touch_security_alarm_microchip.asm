;===============================================================================
; Touch Sensor Based Security Alarm System - Microchip Studio Version
...
.org 0x0002              ; External Interrupt 0 vector (INT0)
...
RESET:
    ...
    ldi temp, (1<<DDB0)|(1<<DDB1)    ; Set PB0 (LED) and PB1 (Buzzer) as outputs
    out DDRB, temp
    
    ; Initialize PORTB outputs to OFF
    ldi temp, 0x00
    out PORTB, temp
    
    ; Configure PORTD (Touch sensor input)
    ldi temp, 0x00       ; Set PORTD.2 as input
    out DDRD, temp
    ldi temp, (1<<PORTD2) ; Enable pull-up on PD2 (INT0)
    out PORTD, temp

    ; Configure External Interrupt 0 (for ATmega32)
    ldi temp, (1<<ISC01)|(1<<ISC00) ; Configure INT0 for rising edge
    out MCUCR, temp      ; Use MCUCR register for ATmega32
    ldi temp, (1<<INT0)  ; Enable INT0
    out GICR, temp       ; Use GICR register for ATmega32
...
BEEP_LOOP:
    sbi PORTB, PORTB1    ; Turn on buzzer
    rcall DELAY          ; Delay
    cbi PORTB, PORTB1    ; Turn off buzzer
    rcall DELAY          ; Delay
    dec temp
    brne BEEP_LOOP
    
    ; Turn off LED after alarm sequence
...
DELAY:                   ; Delay subroutine
    ...
    ldi delay_count1, 200
OUTER_LOOP:
    ldi delay_count2, 200
INNER_LOOP:
